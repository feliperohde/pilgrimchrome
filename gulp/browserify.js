...
export default function(gulp, plugins, args, config, taskTarget, browserSync) {
    ...
    return files.map((entry) => {
      ...
      let rebundle = function() {
        ...
        bundler.bundle()
          ...
          .pipe(gulpif(args.exit === true, exit()));
      };
  ...
  // Browserify Task
  gulp.task('browserify_main', (done) => {
    return glob('./' + path.join(dirs.source, dirs.scripts, entries.main), function(err, files) {
      if (err) {
        done(err);
      }
  ...
  gulp.task('browserify_background', (done) => {
    return glob('./' + path.join(dirs.source, dirs.scripts, entries.background), function(err, files) {
      if (err) {
        done(err);
      }
  ...
  gulp.task('browserify', (done) => {
    gulp.start('browserify_main');
    gulp.start('browserify_background');
  });
